...
export default function RotasPage() {
  ...
  // Carrega as rotas do localStorage quando o componente é montado
  useEffect(() => {
    corrigirContadorRotas()
    loadRotas()
  }, [])

  // Função para corrigir o contador de rotas
  const corrigirContadorRotas = () => {
    const storage = StorageService.getItem<any>("sistema-gestao-data")
    if (storage) {
      // Encontrar o maior ID de rota atual
      let maxId = 0
      if (storage.rotas && storage.rotas.length > 0) {
        storage.rotas.forEach((rota: any) => {
          const rotaId = parseInt(rota.id, 10);
          if (!isNaN(rotaId) && rotaId > maxId) {
            maxId = rotaId;
          }
        });
      }
      
      // Atualizar o contador para ser maior que o maior ID encontrado
      if (!storage.counters) {
        storage.counters = {};
      }
      
      storage.counters.rotas = maxId;
      StorageService.setItem("sistema-gestao-data", storage);
    }
  }

  // Função para carregar as rotas do localStorage
  const loadRotas = () => {
    const storage = StorageService.getItem<{rotas: any[]}>("sistema-gestao-data")
    if (storage && storage.rotas) {
      setRotas(storage.rotas)
    }
  }
  ...
  const handleDelete = () => {
    if (selectedRota) {
      try {
        storageService.deleteRota(selectedRota.id)
        loadRotas() // Recarrega as rotas após a exclusão
        toast({
          title: "Rota excluída",
        ...
        toast({
          ...
          description: error instanceof Error ? error.message : "Ocorreu um erro ao excluir a rota.",
        })
      }
    }
  }
  ...
  const handleAddRota = (data: { rota: string; distancia: string }) => {
    try {
      ...
      loadRotas() // Recarrega as rotas após adicionar
      toast({
  ...
  const handleUpdateRota = (data: { rota: string; distancia: string }) => {
    if (selectedRota) {
      try {
        storageService.updateRota(selectedRota.id, {
          rota: data.rota,
          distancia: data.distancia,
        })
        loadRotas() // Recarrega as rotas após atualizar
        toast({
        ...
        toast({
          ...
          description: error instanceof Error ? error.message : "Ocorreu um erro ao atualizar a rota.",
        })
      }
    }
  }
