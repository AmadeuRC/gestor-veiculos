...
import { storageService, type Employee, type Combustivel, type VeiculoRegistrado } from "@/lib/storage-service"
...
export function NovoAbastecimentoForm({ onSuccess, initialData }: NovoAbastecimentoFormProps) {
  ...
  // Carregar combustíveis cadastrados e outros dados necessários
  useEffect(() => {
    ...
    try {
      ...
      console.log("Funcionários carregados para abastecimento:", loadedFuncionarios)
    ...
    try {
      ...
      console.log("Veículos registrados carregados para abastecimento:", loadedVeiculos)
                      ...
                      {funcionarios.length > 0 ? (
                        funcionarios.map((funcionario) => (
                          <React.Fragment key={funcionario.id}>
                            <SelectItem value={funcionario.name}>
                              {funcionario.name}
                            </SelectItem>
                          </React.Fragment>
                        ))
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                    ...
                    <SelectContent>
                      <SelectItem key="saude" value="saude">SAÚDE</SelectItem>
                      <SelectItem key="educacao" value="educacao">EDUCAÇÃO</SelectItem>
                      <SelectItem key="obras" value="obras">OBRAS</SelectItem>
                      <SelectItem key="assistencia" value="assistencia">ASSISTÊNCIA SOCIAL</SelectItem>
                    </SelectContent>
                  </Select>
                  <FormMessage />
                </FormItem>
                    ...
                    <SelectContent>
                      {combustiveisCadastrados.length > 0 ? (
                        combustiveisCadastrados.map((combustivel) => (
                          <React.Fragment key={combustivel.id}>
                            <SelectItem value={combustivel.combustivel}>
                        ...
                        <FormControl>
                          <RadioGroup onValueChange={field.onChange} value={field.value} className="flex space-x-4">
                            <FormItem className="flex items-center space-x-1 space-y-0">
                              <FormControl>
                                <RadioGroupItem key="sim" value="sim" />
                              </FormControl>
                              <FormLabel className="font-normal">Sim</FormLabel>
                            </FormItem>
                            <FormItem className="flex items-center space-x-1 space-y-0">
                              <FormControl>
                                <RadioGroupItem key="nao" value="nao" />
                              </FormControl>
                        ...
                        <FormItem className="space-y-1">
                          ...
                          <FormControl>
                            <RadioGroup onValueChange={field.onChange} value={field.value} className="flex space-x-4">
                              <FormItem className="flex items-center space-x-1 space-y-0">
                                <FormControl>
                                  <RadioGroupItem key="rota" value="rota" />
                                </FormControl>
                                <FormLabel className="font-normal">Rota</FormLabel>
                              </FormItem>
                              <FormItem className="flex items-center space-x-1 space-y-0">
                                <FormControl>
                                  <RadioGroupItem key="tfd" value="tfd" />
                                </FormControl>
