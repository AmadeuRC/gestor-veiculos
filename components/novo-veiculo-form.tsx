...
import { type VeiculoRegistrado, storageService } from "@/lib/storage-service"
...
const formSchema = z.object({
  ...
  passageiros: z.string().min(1, "A quantidade de passageiros é obrigatória."),
  chassi: z.string().min(1, "O chassi é obrigatório."),
  capacidade: z.string().min(1, "A capacidade é obrigatória."),
  numeroVeiculo: z.string().min(1, "O número do veículo é obrigatório."),
  kmPorLitro: z.string().min(1, "O KM/L é obrigatório."),
  tipoVeiculo: z.string().min(1, "O tipo de veículo é obrigatório."),
...
export function NovoVeiculoForm({ veiculo = null, onSuccess }: NovoVeiculoFormProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: veiculo
      ? {
          ...
          ativo: veiculo.ativo,
          // Outros valores padrão para edição
          ano: "2023",
          cor: "Branco",
          passageiros: "5",
          chassi: "8AC907657RE235510",
          capacidade: "71",
          numeroVeiculo: veiculo.id,
          kmPorLitro: "10",
          propriedade: "Próprio",
          combustivel: "Diesel S-10",
          rodaAro: "Aro 16",
          descricaoPneu: "Pneu 225/75 R16",
  ...
  function onSubmit(data: FormValues) {
    try {
      ...
      const veiculoData = {
        ...
        tipo: `${data.tipoVeiculo} - ${getTipoVeiculoDescricao(data.tipoVeiculo)}`,
        placa: data.placa,
      ...
      if (veiculo) {
        ...
        storageService.addVehicle(veiculoData)
        toast({
      ...
      toast({
        title: "Erro ao salvar",
        description: error instanceof Error ? error.message : "Ocorreu um erro ao salvar o veículo.",
        variant: "destructive",
      })
    }
  }

  // Adicione esta função auxiliar para obter a descrição do tipo de veículo
