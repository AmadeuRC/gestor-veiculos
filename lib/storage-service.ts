...
export class StorageService {
   ...
   */
  public static getEmptyStorage(): Storage {
    return {
      users: [],
      vehicles: [],
      brands: [],
   ...
   */
  public static getNextId(type: string): string {
  ...
  /**
   * Corrige os IDs de funcionários, garantindo que sejam únicos
   * Esta função deve ser executada se houver problemas com IDs duplicados
   */
  public static corrigirContadorFuncionarios(): void {
    const storage = StorageService.getItem<Storage>(STORAGE_KEY) || StorageService.getEmptyStorage()
    ...
    // Ordena os funcionários por ID para facilitar a verificação
    storage.employees.sort((a, b) => parseInt(a.id) - parseInt(b.id))

    // Verifica e corrige IDs duplicados
    let lastId = 0
    const idsProcessados = new Set<string>()

    storage.employees = storage.employees.map(funcionario => {
      // Se o ID já foi processado (duplicado), atribui um novo ID
      if (idsProcessados.has(funcionario.id)) {
        lastId++
        return { ...funcionario, id: lastId.toString() }
      }

      // Adiciona o ID ao conjunto de processados
      idsProcessados.add(funcionario.id)
      
      // Atualiza o lastId para ser o maior ID encontrado
      const idNumber = parseInt(funcionario.id)
      if (idNumber > lastId) {
        lastId = idNumber
      }
    ...
    // Atualiza o contador para ser o próximo após o maior ID
    storage.counters.employees = lastId + 1

    // Salva as alterações
    StorageService.setItem(STORAGE_KEY, storage)
    StorageService.addLog("Correção", "Sistema", `Contador de funcionários corrigido para ${storage.counters.employees}`)
    
    console.log(`Correção de IDs de funcionários concluída. Próximo ID: ${storage.counters.employees}`)
  }

  /**
   * Atualiza um funcionário existente
   * @param id ID do funcionário a ser atualizado
   * @param employee Dados parciais do funcionário para atualização
   * @returns Funcionário atualizado
   * @throws Error se não encontrar o funcionário
   */
  public static updateEmployee(id: string, employee: Partial<Employee>): Employee {
    const storage = StorageService.getItem<Storage>(STORAGE_KEY) || StorageService.getEmptyStorage()

    let funcionarioAtualizado: Employee | null = null

    storage.employees = storage.employees.map((f: Employee) => {
      if (f.id === id) {
        funcionarioAtualizado = { ...f, ...employee }
    ...
    StorageService.setItem(STORAGE_KEY, storage)
    StorageService.addLog("Atualização", "Administrador", `Funcionário ${funcionarioAtualizado.name} atualizado`)
...
export const storageService = {
  ...
  addVehicle: (vehicle: any): any => {
    try {
      ...
      const newId = StorageService.getNextId("veiculosData")
      console.log("Gerando novo ID para veículo:", newId)
      ...
      console.log("Veículo adicionado ao array:", newVehicle)
      console.log("Array atualizado:", storage.veiculosData)

      // Salvar no storage
      StorageService.setItem(STORAGE_KEY, storage)

      // Verificar se o veículo foi realmente salvo
      const updatedStorage = StorageService.getItem<Storage>(STORAGE_KEY)
      console.log("Storage após salvar:", updatedStorage)
      console.log("veiculosData após salvar:", updatedStorage?.veiculosData)
  ...
  saveAbastecimento: (abastecimento: Omit<Abastecimento, "id">): Abastecimento => {
    ...
    // Determinar o próximo ID com base no maior ID existente
    const maxId = storage.abastecimentos.length > 0
      ? Math.max(...storage.abastecimentos.map(a => parseInt(a.id) || 0))
      : 0;
  ...
  saveCombustivel: (combustivel: Omit<Combustivel, "id" | "acumulado" | "saldo">): Combustivel => {
    ...
    // Determinar o próximo ID com base no maior ID existente
    const maxId = storage.combustiveis.length > 0
      ? Math.max(...storage.combustiveis.map(c => parseInt(c.id) || 0))
      : 0;
    const nextId = (maxId + 1).toString();
    ...
    const newCombustivel: Combustivel = {
      ...
      observacoes: combustivel.observacoes || ""
    }
  ...
  getVeiculos: (): any[] => {
    try {
      const storage = StorageService.getItem<Storage>(STORAGE_KEY) || StorageService.getEmptyStorage()
      console.log("StorageService.getVeiculos - storage completo:", storage)
      console.log("StorageService.getVeiculos - veiculosData:", storage.veiculosData)
  ...
  deleteEmployee: (id: string): void => {
    ...
    const funcionarioParaExcluir = storage.employees.find((f: Employee) => f.id === id)

    if (!funcionarioParaExcluir) {
      throw new Error(`Funcionário com ID ${id} não encontrado.`)
    }

    storage.employees = storage.employees.filter((f: Employee) => f.id !== id)

    StorageService.setItem(STORAGE_KEY, storage)
    StorageService.addLog(
      "Exclusão",
      "Administrador",
      `Funcionário ${funcionarioParaExcluir.name} excluído`
    )
  },
}
